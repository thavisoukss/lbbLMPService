# LMPS Service — Antigravity (AGY) Instructions
...
Every controller method must log START, request body, END, final response, and duration_ms — no exceptions. Financial transactions require an audit trail at every entry point, enabling traceability and duration monitoring without attaching a debugger.
...
Every service method must log key events throughout its execution flow. Controllers capture entry/exit; services hold the business logic where failures actually occur — validation, external calls, DB writes. Without these logs, ops cannot pinpoint which step failed or how long each step took.
...
### Logging
Use `SLF4J` / `Log4j2` for all logging.
...
### Database Access
Use Oracle JDBC (`ojdbc8`) with JPA. Apply `@Transactional` at the service layer — read-only queries use `@Transactional(readOnly = true)`.
...
### External API Calls
Use `RestClient` for all calls to external APIs with connect-timeout 5 s and read-timeout 10 s.
...
After any correction from the user, update `.agents/lessons.md` with the pattern and a rule to prevent recurrence.
- **Why**: Repeating the same mistake signals a missing rule, not a one-off error. Persistent rules eliminate classes of mistakes.
- **How**: Record the pattern that caused the mistake, the rule that prevents it, and review `.agents/lessons.md` at the start of each session.

### 3. Verification Before Done
Never claim a change is complete without running verification and showing evidence.
...
For non-trivial changes, pause and evaluate whether a more elegant solution exists before finalizing.
...
### 5. Skills Usage
Use skills for any task that matches an available capability.
- **Why**: Skills encapsulate specialized patterns and prevent reinventing solved problems.
- **How**: Load skills from `.agents/skills/`. Invoke with natural language. Treat each skill as one independent capability.

### 6. Subagents Usage
Delegate complex or isolated subtasks to subagents.
- **Why**: Subagents keep the main context window clean and allow focused execution per tech area.
- **How**: Load subagents from `.agents/agents/`. Assign one task per subagent. For complex problems, use multiple subagents in parallel.

### 7. No Magic
All assumptions must be stated explicitly. Never invent infrastructure, services, or behavior that wasn't specified.
...
### 8. Dissent
Before any major change, surface concerns out loud before touching code.
...
Track stated goals vs actual execution. Flag scope creep immediately.
...
Classify every action by reversibility before executing it.
